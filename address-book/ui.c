...
MY_MENU PrintMenu() {
	...
	printf("[1]Search\t[2]Insert\t[3]Update\t[4]Delete\t[5]PrintNode\t[6]PrintFile\t[0]Exit\n");
	scanf_s("%d%*c", &input);
	return input;
}
...
void Insert(void) {
	...
	printf("%s, %s, %s, %s 사용자가 정상적으로 추가됐습니다.\n", name, address, phone, active);
	PrintList();
}
...
void Update(void) {
	...
	int offset = 0;
	USERDATA* pTmp = SearchByNameAndPhone(name, phone, &offset);
	if (pTmp == NULL) {
		puts("기존에 해당 사용자가 없습니다.");
	}
	else UpdateToFile(newName, newAddress, newPhone, active, pTmp, offset - 1);
}
...
void Delete(void) {
	...
	scanf_s("%s%*c", phone, 32);
	int offset = 0;
	USERDATA* pTmp = SearchToRemove(phone, &offset);
	if (pTmp == NULL) {
